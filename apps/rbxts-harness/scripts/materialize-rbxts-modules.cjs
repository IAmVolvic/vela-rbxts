...
// into the place, and the hoisted root scope holds every scope package any
// workspace member pulled — copying it wholesale ships this place the ones
// belonging to the other harnesses.
...
// Their versions stand still while their builds change, so these are refreshed
// every run.
for (const name of ["runtime", "runtime-core"]) {
