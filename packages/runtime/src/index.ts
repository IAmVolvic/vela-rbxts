...
/// Mirrors `opacity_transparency_props`: every channel the host paints itself.
/// A CanvasGroup composites its whole subtree, so `GroupTransparency` is the one
/// property that already means what CSS `opacity` means; everywhere else the
/// background alone is invisible on a label, whose background is transparent.
function opacityTransparencyProps(tag: string | undefined): string[] {
	if (tag === "canvasgroup") {
...
function composeInheritedOpacity(
	resolution: RuntimeResolution,
	tag: string | undefined,
	alpha: number,
) {
	for (const name of opacityTransparencyProps(tag)) {
		const current = resolution.props[name];
		if (current === undefined) {
			continue;
		}
		resolution.props[name] = 1 - (1 - (current as number)) * alpha;
	}
...
function resolveUtilityToken(
	...
	if (startsWith(token, "opacity-")) {
		...
		return propsEffect(
			opacityTransparencyProps(tag).map((name) => ({ name, value })),
		);
	}
