...
namespace __VelaOpacity {
	...
	function applyToElement(
		...
		const names = transparencyProps(tag);
		const children = props.children;
		...
		for (const name of names) {
			...
			faded[name] = compose(typeIs(current, "number") ? current : 0, alpha);
		}
	...
	return __VelaReact.forwardRef(
		(props: VelaRuntimeHostProps, forwardedRef: unknown) => {
			...
			if (hostTag !== undefined && ambientAlpha < 1) {
				for (const name of __VelaOpacity.transparencyProps(hostTag)) {
					if (resolution.props[name] !== undefined) {
						continue;
					}

					const current = hostProps[name];
					hostProps[name] = __VelaOpacity.compose(
			...
			if (transition !== undefined) {
				...
				for (const [name, value] of pairs(hostProps)) {
					...
					if (
						margin !== undefined &&
						__VelaMargin.isMarginWrapperProp(name as string)
					) {
						continue;
					}
					if (
			...
			const wrapperProps =
				margin !== undefined
					? __VelaMargin.prepareMarginWrapper(margin, hostProps)
					: undefined;
